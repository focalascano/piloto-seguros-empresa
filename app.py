...
def generar_anexo_pdf(seguros_activos, nivel):
    ...
    pdf.chapter_body(TEXTOS_LEGALES["GENERAL"]["encabezado"])

    if not seguros_activos:
        pdf.chapter_body("No se han determinado seguros específicos adicionales bajo el nivel de Riesgo Nulo.")
    else:
...
def generar_checklist_pdf(seguros_activos, nivel, respuestas):
    ...
    pdf.cell(0, 10, 'CHECKLIST DE CONTROL DOCUMENTAL', 0, 1, 'C')
    pdf.ln(5)
    ...
    if not seguros_activos:
        pdf.chapter_body("Sin seguros adicionales requeridos.")
    else:
        for s in seguros_activos:
...
respuestas_dict = {"P1": r1, "P2": r2, "P3": r3, "P4": r4, "P5": r5, "P6": r6, "P7": r7, "P8": r8, "P9": r9}

errores = []
if not p1 and (p3 or p4 or p5 or p6 or p7 or p8 or p9):
    errores.append("⚠️ Bloqueo: Requiere P1 = SÍ.")
if p2 and (p4 or p5 or p6 or p7 or p8 or p9):
    errores.append("⚠️ Bloqueo: Administrativo incompatible con riesgos.")
