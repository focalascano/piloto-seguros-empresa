...
**Implementación informatica del moodelo:** 
Fernanda Lascano
**Fecha:** 2026""")
...
if st.button("GENERAR ANEXO DE SEGUROS"):
    if errores:
        ...
        nivel = "Nulo" if not p1 else ("Alto" if (p9 or p8 or p5 or p4) else ("Medio" if p7 else "Bajo"))
        st.info(f"Nivel de Riesgo Determinado: {nivel}")
        
        if nivel != "Nulo":
            seguros_para_word = []
            if p1:
                seguros_para_word.append({'clausula': f"{TEXTOS_LEGALES['ART']}\n\n{TEXTOS_LEGALES['VO']}\n\n{TEXTOS_LEGALES['AP']}"})
                if p5 or p7 or p8 or p9:
                    suma = "USD 100.000 (o eq. local)" if nivel == "Alto" else "USD 50.000 (o eq. local)"
                    seguros_para_word.append({'clausula': TEXTOS_LEGALES["RC"], 'suma': suma})
            if p4: seguros_para_word.append({'clausula': TEXTOS_LEGALES["CAUCION"]})
            if p9: seguros_para_word.append({'clausula': TEXTOS_LEGALES["TRCYM"]})
